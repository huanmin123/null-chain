...
- **支持 Fun 类型和高阶函数**（函数作为参数、函数返回函数、Lambda 表达式）
- **支持 Lambda 表达式**（匿名函数、闭包、函数组合）
- 支持任务调用和并发执行
...
NF 脚本支持完整的函数式编程能力，包括 Fun 类型、Lambda 表达式、高阶函数等。

详细文档请参考：**[NF 脚本函数和 Fun 类型](./15-NF脚本函数和Fun类型.md)**
...
// 高阶函数：函数作为参数
fun apply(Fun<Integer, Integer : Integer> op, Integer a, Integer b) Integer {
    return op(a, b)
}

// 调用
Integer result = apply((a, b) -> { return a + b }, 10, 20)
echo "10 + 20 = {result}"
```
...
```java
String mathScript = "fun add(Integer a, Integer b)Integer {\n" +
                    "    return a + b\n" +
                    "}";
NfScriptRegistry.registerScript("math", mathScript);

String mainScript = "import nf math\n" +
                    "export math.add(10, 20)";
...
```nf
// 先导入任务并起别名
import task com.example.task.UserTask as userTask
import task com.example.task.ScoreTask as scoreTask
...
// 只执行任务，不接收返回值
run userTask(name, age)

// 绑定单任务返回值
run userTask(name, age) -> result:String

// 参数支持变量和简单字面量
run userTask(name, 18, greeting, true, 3.5)

// 多任务并发执行并返回 Map
run userTask(name, age), scoreTask(95, true) -> results:Map
```

**说明**
- `run` 只能调用已通过 `import task ... as 别名` 导入的任务别名
- 参数当前支持变量和简单字面量：字符串、模板字符串、整数、浮点数、`true`、`false`
- `run` 参数里**不支持**直接写任意表达式或函数调用；复杂参数请先赋值到变量，再传给任务
- 同一条并发 `run` 语句里不能重复写同名任务别名
- `run task1(...), task2(...) -> results:Map` 返回的 `Map`，key 是任务完整类名，不是 `as` 别名
...
```java
String script = "Integer total = 10\n" +
                "export total";
...
Object result = NfMain.runLocal(filePath, log, null);
```
...
10. **Fun 类型和 Lambda 表达式**：
    - NF 脚本**支持** Fun 类型和 Lambda 表达式（详细文档：[NF 脚本函数和 Fun 类型](./15-NF脚本函数和Fun类型.md)）
    - Lambda 语法：`(params) -> { body }`
    - **不支持** Java 8 的简化 Lambda 语法（如 `item -> echo item`）
    - **Java 函数式接口**：NF 的 `Fun` / Lambda 在很多场景下可以自动转换为 Java 的 `Function<T,R>`、`Predicate<T>` 等参数
    - **错误示例**：
      ```nf
      // ✗ 不支持 Java 8 的简化 Lambda 语法
      list.forEach(item -> echo item)
      ```
    - **正确做法（自动转换到 Java 函数式接口）**：
      ```nf
    ...
    - **正确做法（inline Lambda）**：
      ...
      ```
    - **兜底方案**：如果 Java 方法重载过多或自动转换目标不明确，仍可以回退到显式 Java 实现类
11. **导入 NF 脚本**：
    - `import nf xxx` 之前，需要先通过 `NfScriptRegistry.registerScript("xxx", scriptContent)` 注册脚本
    - 建议统一在应用启动阶段完成注册，避免运行时缺失
12. **任务调用参数**：
    - `run` 参数当前只支持变量和简单字面量
    - 如果需要传 `a + b`、函数调用结果或复杂对象表达式，请先赋值到变量
13. **并发任务结果**：
    - `run task1(...), task2(...) -> results:Map` 返回的 `Map` key 是任务完整类名
    - 如果业务侧需要更稳定的业务键名，建议脚本里自行组装一个新的 `Map` 后再 `export`
...
- **[NF 脚本函数和 Fun 类型](./15-NF脚本函数和Fun类型.md)** - 函数定义、Fun 类型、Lambda 表达式、高阶函数、闭包等详细说明
