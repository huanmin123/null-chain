...
try {
    String name = Null.of(user)
        .map(User::getRole)
        .map(Role::getName)
        .get();   //如果name为空，异常信息会显示：Null.of->map->map? 会精确定位到具体的位置
} catch (Exception e) {
...
**使用 Null-Chain 解决（短路模式）：**

```java
// 使用 Null-Chain 的 of() 方法 - 遇到第一个空就终止，适合多级链路过程中需要处理空的情况
// 如果 user 为 null，后续操作自动跳过
String name = Null.of(user)
    .of(User::getName)      // 如果 name 为空，链变为空链，后续操作跳过
    .map(String::toUpperCase)
    .orElse("未知名称");

// 每个字段独立处理
String email = Null.of(user)
    .map(User::getEmail)
    .orElse("无邮箱");

String phone = Null.of(user)
    .map(User::getPhone)
    .orElse("无电话");
```

**使用 NullCheck 批量判空（全部处理模式）：**

```java
// 使用 NullCheck - 全部处理一遍，然后日志会告诉你哪些为空
// 适合方法入参校验时快速处理，会检查所有字段并统一报告
Null.ofCheck(user)
    .of(User::getName)      // 检查并记录
    .of(User::getEmail)     // 继续检查并记录
    .of(User::getPhone)     // 继续检查并记录
    .doThrow(IllegalArgumentException.class, "用户信息不完整");
// 异常信息会包含完整的链路信息，显示哪些字段为空，如：user->name?->email?->phone
```

**两种方式的区别：**

| 特性 | Null-Chain.of() | NullCheck |
|------|----------------|-----------|
| **处理方式** | 遇到第一个空就终止（短路模式） | 全部处理一遍 |
| **适用场景** | 多级链路过程中需要处理空的情况 | 方法入参校验时快速处理 |
| **日志信息** | 显示到第一个空值的位置 | 显示所有为空的位置 |
| **性能** | 性能更好（短路） | 需要全部检查 |
| **使用目的** | 获取值或执行操作 | 验证数据完整性 |
...
**场景说明**：需要先访问嵌套对象（如 `user.getAddress()`），然后确保该对象的多个平级字段（如 `address.getCity()` 和 `address.getStreet()`）都不为空时，才返回另一个字段（如 `address.getZipCode()`）。

**传统代码的问题：**

```java
// 传统方式 - 嵌套和平级判空混合，代码非常冗长
String zipCode = null;

if (user != null) {
    Address address = user.getAddress();
    if (address != null) {
        // 需要确保 city 和 street 都不为空，才获取 zipCode
...
```java
// 场景1：确保 address.getCity() 和 address.getStreet() 都不为空时，才返回 address.getZipCode()
// 使用 of() 方法：如果字段为空，链变为空链（中断）
String zipCode = Null.of(user)
    .map(User::getAddress)           // 嵌套访问：user -> address
    .of(Address::getCity)            // 平级判空：如果 city 为空，链变为空链
    .of(Address::getStreet)          // 平级判空：如果 street 为空，链变为空链
    .map(Address::getZipCode)        // 只有 city 和 street 都不为空时，才获取 zipCode
    .orElse("未知邮编");

// 或者使用 ifGo() 配合条件判断
String zipCode = Null.of(user)
    .map(User::getAddress)           // 嵌套访问：user -> address
    .ifGo(a -> a.getCity() != null && a.getStreet() != null)  // 确保 city 和 street 都不为空
    .map(Address::getZipCode)        // 获取 zipCode
    .orElse("未知邮编");
```

**场景2：如果需要同时获取  Address、ZipInfo 使用收集器**

```java
// 使用收集器一次性获取多个值
// 先获取 city 和 street，确保它们都不为空
NullCollect collect = Null.of(user)
        .map(User::getAddress)
        .of(Address::getCity)           // 确保 city 不为空
        .of(Address::getStreet)         // 确保 street 不为空
        .of(Address::getZipCode)        // 获取 zipCode
        .map(this::getZipCodeInfo)      // 通过自定义方法获取 zipCode 的名称 (返回的是对象)
        .collect();                     // 加载收集器
// 从收集器中获取所有值
NullChain<Address> address = collect.get(Address.class);
// 获取 ZipInfo
NullChain<String> zipCode = collect.get(ZipInfo.class);  
```
